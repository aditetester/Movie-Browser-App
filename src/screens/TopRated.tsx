...
import {useGetTopRatedMoviesMutation} from '../services/movieApi';
...
const TopRated = () => {
  ...
  const [getTopRatedMovies] = useGetTopRatedMoviesMutation();

  const getTopRatedMoviesHandler = useCallback(async () => {
    setIsLoading(true);
    try {
      const res: MoviesResponse = await getTopRatedMovies({}).unwrap();
      if (res) {
        setMovies(res);
      }
    } catch (error) {
