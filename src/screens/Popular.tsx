...
import {useGetPopularMoviesMutation} from '../services/movieApi';
...
const Popular = () => {
  ...
  const getPopularMoviesHandler = useCallback(async () => {
    setIsLoading(true);
    try {
      const res: MoviesResponse = await getPopularMovies({}).unwrap();
      if (res) {
        setMovies(res);
      }
    } catch (error) {
